...
def pivot_table(ratings_list):
    ...
    parameters:
        ratings_list [list of dicts]:
            represents the table BookRatings
    ...
    ysort = []
    # xsort is list of all book ISBNs
    xsort = []
    # loop through every row in BookRatings table representation
    for row in ratings_list:
        ...
        if yaxis not in ysort:
            ysort.append(yaxis)
        # xaxis is current row's book ISBN
        xaxis = row["ISBN"]
        # if book ISBN is not yet in xsort, it saves to xsort
        ...
        try:
            ...
            result[yaxis] = {}
        # tests if book ISBN is in results dictionary under user ID key
        if xaxis not in result[yaxis]:
            # if not, add 0 to create placeholder under user ID, book ISBN
            result[yaxis][xaxis] = 0
        # saves Rating in results dictionary under user ID and book ISBN
        result[yaxis][xaxis] = row["Rating"]
    # loops through all user IDs
    for yaxis in ysort:
        # loops through all book ISBNs
        for xaxis in xsort:
            # if book ISBN does not already exist within user ID,
            #     saves as 0 rating
            if xaxis not in result[yaxis]:
                result[yaxis][xaxis] = 0
    ...
    headings = ["ID"]
    # extends headings list with all book ISBNs
    ...
    for user_id in ysort:
        ...
        row = [user_id]
        # extends row with user ratings for each book ISBN
...
def to_matrices(pivot_table, user_id):
    ...
    Turns a list of dicts pivot table into matrices
        representing specific user ratings of books
        and all other user ratings of books
    ...
    returns:
        matrix [list of lists]:
            matrix representation of pivot table,
                all users except specific user
        user_row [list of lists]:
            matrix representation of the pivot table,
                specifically for user's ratings
        headings [list]:
            list of just headings with all book ISBNs
    """
    ...
    for ISBN in pivot_table[0].keys():
        if ISBN != "ID":
            headings.append(ISBN)
    ...
    returns:
        data [numpy.ndarray]:
            numpy array representation of the pivot table,
                all users except user finding recommendation for
        user [numpy.ndarray]:
            numpy array representation of the pivot table,
                specifically for user's ratings
    """
